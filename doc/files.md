...
Windows  | `%LOCALAPPDATA%\Qtum\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Qtum blocks (dumped in network format, 128 MiB per file)
...
`qtum-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).
...
This table describes the files installed by Qtum Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| qtum.conf                                               | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
| bin/qtum                                                | Command-line tool for interacting with Qtum. Calls other binaries below. |
| bin/qtum-cli                                            | Tool for making node and wallet RPC calls.                                  |
| bin/qtum-qt                                             | Qtum node and wallet GUI                                                 |
| bin/qtum-tx                                             | Tool for creating and modifying transactions                                |
| bin/qtum-util                                           | Miscellaneous utilities                                                     |
| bin/qtum-wallet                                         | Qtum wallet tool                                                         |
| bin/qtumd                                               | Qtum node and wallet daemon                                              |
...
| *libexec/bench_qtum*                                    | Benchmarking tool for measuring node performance                            |
| *libexec/qtum-chainstate*                               | Tool to validate and connect blocks                                         |
| libexec/qtum-gui                                        | IPC-enabled alternative to `qtum-qt`                                     |
| libexec/qtum-node                                       | IPC-enabled alternative to `qtumd`                                       |
| libexec/test_qtum                                       | Unit test binary                                                            |
| *libexec/test_qtum-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `qtum-cli`, `qtumd`, and others |
...
- README and qtum.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `qtum-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Qtum Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
