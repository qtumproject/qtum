...
| *libbitcoin_ipc*         | IPC functionality used by *qtum-node* and *qtum-gui* executables to communicate when [`-DENABLE_IPC=ON`](multiprocess.md) is used. |
...
- *libbitcoin_common* is a home for miscellaneous shared code used by different Qtum Core applications. It should not depend on anything other than *libbitcoin_util*, *libbitcoin_consensus*, and *libbitcoin_crypto*.
