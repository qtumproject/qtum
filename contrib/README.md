...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Qtum network. This means one can have an always-on qtumd instance running, and another local qtumd/qtum-qt instance which connects to this node and receives blocks from it.
...
Utilities to generate test vectors for the data-driven Qtum tests.
...
This script converts a compact-serialized UTXO set (as generated by Qtum Core with `dumptxoutset`)
