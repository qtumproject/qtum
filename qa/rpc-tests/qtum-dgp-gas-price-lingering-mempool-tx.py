...
class DGPState:
    ...
    def send_set_initial_admin(self, sender):
        ...
        self.node.sendtocontract(self.contract_address, self.abiSetInitialAdmin, 0, 4000000, 0.0001, sender)

    def send_add_address_proposal(self, proposal_address, type1, sender):
        self.node.sendtoaddress(sender, 1)
        self.node.sendtocontract(self.contract_address, self.abiAddAddressProposal + proposal_address.zfill(64) + hex(type1)[2:].zfill(64), 0, 2000000, 0.001, sender)
...
class QtumDGPGasPriceLingeringMempoolTxTest(BitcoinTestFramework):
    ...
    def create_proposal_contract(self, min_gas_price):
        ...
        if min_gas_price == 10:
            contract_data = self.node.createcontract("", 1000000, 0.001)
    ...
    def run_test(self):
        ...
        self.nodes[1].createcontract("00", 1000000, 0.00000001)

        # Set the minimum gas price to 10
        self.create_proposal_contract(10)
