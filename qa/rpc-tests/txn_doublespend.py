...
class TxnMallTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # We need the entire node's balance
        spend_from_foo = int(starting_balance - INITIAL_BLOCK_REWARD*5)
        spend_from_bar = int(INITIAL_BLOCK_REWARD*5 - 100)
        ...
        txid1 = self.nodes[0].sendfrom("foo", node1_address, int(INITIAL_BLOCK_REWARD / 5)*4, 0)
        txid2 = self.nodes[0].sendfrom("bar", node1_address, int(INITIAL_BLOCK_REWARD / 5)*2, 0)
        ...
        # negative):
        expected = starting_balance + 2*int(INITIAL_BLOCK_REWARD) - spend_from_doublespend + fund_foo_tx["fee"] + fund_bar_tx["fee"] + doublespend_fee
        assert_equal(self.nodes[0].getbalance(), expected)
        assert_equal(self.nodes[0].getbalance("*"), expected)
