...
class QtumEVMcancunTest(BitcoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1
        self.extra_args = [['-txindex', '-logevents=1', '-londonheight=10000', '-shanghaiheight=1000000', '-cancunheight=100000', '-pectraheight=100000', '-osakaheight=100000']]
    ...
    def run_test(self):
        ...
        # Pre cancun fork tests
        switch_height = self.node.getblockcount()
        self.log.info('Runnig tests before cancun (height: %s)', switch_height)
        ...
        # Post cancun fork tests
        switch_height = self.node.getblockcount()
        self.log.info('Runnig tests after cancun (height: %s)', switch_height)
        self.restart_node(0, ['-txindex', '-logevents', '-cancunheight={}'.format(switch_height)])
...
if __name__ == '__main__':
    QtumEVMcancunTest(__file__).main()
