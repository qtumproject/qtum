...
class QtumEVMLondonTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        generatesynchronized(self.node, COINBASE_MATURITY + 500, None, self.nodes[0:7])
        ...
        for i in range(1, self.num_nodes):
            ...
            generatesynchronized(self.node, 6, None, self.nodes[0:7])
            self.nodes[i].splitutxosforaddress(self.node_addresses[i-1], 100, 100, 10000)
            generatesynchronized(self.nodes[i], 6, None, self.nodes[0:7])     
        
        generatesynchronized(self.node, COINBASE_MATURITY+100, None, self.nodes[0:7])
        for n in self.nodes: n.setmocktime(int(time.time()))
        
        next_possible_fork_height = 6048
        self.log.info("Advancing to fork height -10")
        generatesynchronized(self.node, next_possible_fork_height-self.node.getblockcount()-10, None, self.nodes[0:7])
