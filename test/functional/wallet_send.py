...
class WalletSendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=600, estimate_mode="unset", add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, Decimal(len(res["hex"]) / 2), Decimal("0.006"))

        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_fee_rate=453.2, add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, Decimal(len(res["hex"]) / 2), Decimal("0.004532"))

        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_fee_rate=500, add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        assert_fee_amount(fee, Decimal(len(res["hex"]) / 2), Decimal("0.005"))

        # Test that passing fee_rate as both an argument and an option raises.
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_fee_rate=400, fee_rate=400, add_to_wallet=False,
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=399.99999999,
            expect_error=(-4, "Fee rate (399.999 sat/vB) is lower than the minimum fee rate setting (400.000 sat/vB)"))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_fee_rate=399.99999999,
            expect_error=(-4, "Fee rate (399.999 sat/vB) is lower than the minimum fee rate setting (400.000 sat/vB)"))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=0,
            expect_error=(-4, "Fee rate (0.000 sat/vB) is lower than the minimum fee rate setting (400.000 sat/vB)"))
