...
class QtumSoftMinerGasRelatedLimitsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for node_that_cannot_mine_the_txs in self.nodes[1:3]:
            block_hash = self.generate(node_that_cannot_mine_the_txs, 1)[0]
        ...
        block_hash = self.generate(self.nodes[2], 1)[0]
        assert_equal(len(self.nodes[2].getblock(block_hash)['tx']), 1)
        assert_equal(len(self.nodes[2].getrawmempool()), 5)
        self.sync_all()

        block_hash = self.generate(self.nodes[1], 1)[0]
        ...
        block_hash = self.generate(self.nodes[3], 1)[0]
