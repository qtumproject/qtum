...
class ImportDescriptorsTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-addresstype=legacy", "-txindex"],
                           ["-addresstype=bech32", "-keypool=5", "-txindex"]
                          ]
    ...
    def run_test(self):
        ...
        self.sync_blocks()
        decoded_tx = w1.getrawtransaction(txid, True)
        vout_index = 0 if decoded_tx['vout'][0]['scriptPubKey']['addresses'][0] == address else 1
        tx = wpriv.createrawtransaction([{"txid": txid, "vout": vout_index}], {w0.getnewaddress(): 49.999})
