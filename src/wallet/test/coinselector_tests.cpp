...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        const auto q_result1 = KnapsackSolver(KnapsackGroupOutputs(available_coins.All(), *wallet, filter_confirmed), 5 * CENT, CENT);
        BOOST_CHECK(!q_result1);
        const auto q_result2 = KnapsackSolver(KnapsackGroupOutputs(available_coins.All(), *wallet, filter_confirmed), 195 * CENT + GetRand<unsigned int>(100) + 1, CENT);
