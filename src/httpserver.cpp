...
HTTPRequest::HTTPRequest(struct evhttp_request* _req, const util::SignalInterrupt& interrupt, bool _replySent)
    : req(_req), m_interrupt(interrupt), replySent(_replySent), startedChunkTransfer(false), connClosed(false)
{
}
...
void HTTPRequest::waitClientClose() {
    LogDebug(BCLog::HTTPPOLL, "wait for connection close\n");
    ...
    if (isConnClosed()) {
        LogDebug(BCLog::HTTPPOLL, "wait for connection close, ok\n");
    } else if (!IsRPCRunning()) {
        LogDebug(BCLog::HTTPPOLL, "wait for connection close, RPC stopped\n");
    } else {
        LogDebug(BCLog::HTTPPOLL, "wait for connection close, timeout after 5 seconds\n");
    }
}

void HTTPRequest::startDetectClientClose() {
    LogDebug(BCLog::HTTPPOLL, "start detect http connection close\n");
   ...
   evhttp_connection_set_closecb(conn, [](struct evhttp_connection *conn, void *data) {
       LogDebug(BCLog::HTTPPOLL, "http connection close detected\n");
