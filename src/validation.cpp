...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        QtumTxConverter converter(tx, m_pool, NULL, NULL, contractflags);
...
bool CheckHeaderPoS(const CBlockHeader& block, const Consensus::Params& consensusParams, ChainstateManager& chainman)
{
    LOCK(cs_main);
    // Check for proof of stake block header
    // Get prev block index
    BlockMap::iterator mi = chainman.BlockIndex().find(block.hashPrevBlock);
    ...
    if(pindexPrev->nHeight >= consensusParams.nEnableHeaderSignatureHeight && !CheckRecoveredPubKeyFromBlockSignature(pindexPrev, block, chainman.ActiveChainstate().CoinsTip(), chainman)) {
        return error("Failed signature check");
    }

    return CheckKernel(pindexPrev, block.nBits, block.StakeTime(), block.prevoutStake, chainman.ActiveChainstate().CoinsTip(), chainman);
}

bool CheckHeaderProof(const CBlockHeader& block, const Consensus::Params& consensusParams, ChainstateManager& chainman){
    if(block.IsProofOfWork()){
        return CheckHeaderPoW(block, consensusParams);
    }
    if(block.IsProofOfStake()){
        return CheckHeaderPoS(block, consensusParams, chainman);
    }
    return false;
}
...
bool GetSpentCoinFromMainChain(const CBlockIndex* pforkPrev, COutPoint prevoutStake, Coin* coin, ChainstateManager& chainman) {
    const CBlockIndex* pforkBase = chainman.ActiveChain().FindFork(pforkPrev);

    // If the forkbase is more than coinbaseMaturity blocks in the past, do not attempt to scan the main chain.
    int nHeight = chainman.ActiveChain().Tip()->nHeight;
...
bool CheckReward(const CBlock& block, BlockValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount gasRefunds, CAmount nActualStakeReward, const std::vector<CTxOut>& vouts, CAmount nValueCoinPrev, bool delegateOutputExist, ChainstateManager& chainman)
{
    ...
    else
    {
        ...
        if(!GetMPoSOutputs(mposOutputList, splitReward, nPrevHeight, consensusParams, chainman))
...
valtype GetSenderAddress(const CTransaction& tx, const CCoinsViewCache* coinsView, const std::vector<CTransactionRef>* blockTxs, const CTxMemPool& mempool, int nOut = -1){
    CScript script;
    ...
    if(!scriptFilled)
    {
        ...
        uint256 hashBlock;
        txPrevout = GetTransaction(nullptr, &mempool, tx.vin[0].prevout.hash, Params().GetConsensus(), hashBlock, true);
...
UniValue vmLogToJSON(const ResultExecute& execRes, const CTransaction& tx, const CBlock& block, ChainstateManager &chainman){
    ...
    if(block.GetHash() != CBlock().GetHash()){
        ...
        result.pushKV("blockheight", chainman.ActiveChain().Tip()->nHeight + 1);
    } else {
        result.pushKV("time", GetAdjustedTime());
        result.pushKV("blockheight", chainman.ActiveChain().Tip()->nHeight);
    }
...
void writeVMlog(const std::vector<ResultExecute>& res, ChainstateManager &chainman, const CTransaction& tx, const CBlock& block){
    ...
    for(size_t i = 0; i < res.size(); i++){
        ss << vmLogToJSON(res[i], tx, block, chainman).write();
        if(i != res.size() - 1){
            ss << ",";
        } else {
            ss << "]}";
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(QtumTransaction& tx : txs){
        ...
        result.push_back(globalState->execute(envInfo, *globalSealEngine.get(), tx, chainman, type, OnOpFunc()));
    }
...
bool ByteCodeExec::processingResults(ByteCodeExecResult& resultBCE){
    ...
    for(size_t i = 0; i < result.size(); i++){
        ...
        if(result[i].execRes.excepted != dev::eth::TransactionException::None){
        	...
        	if(!(chainman.ActiveChain().Height() >= consensusParams.QIP7Height && result[i].execRes.excepted == dev::eth::TransactionException::RevertInstruction)){
        	resultBCE.usedGas += gasUsed;
        	}
        }

        if(result[i].execRes.excepted == dev::eth::TransactionException::None || (chainman.ActiveChain().Height() >= consensusParams.QIP7Height && result[i].execRes.excepted == dev::eth::TransactionException::RevertInstruction)){
...
bool QtumTxConverter::extractionQtumTransactions(ExtractQtumTX& qtumtx){
    ...
    refundSender = dev::Address(GetSenderAddress(txBit, view, blockTransactions, mempool));
...
QtumTransaction QtumTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    ...
    dev::Address sender(GetSenderAddress(txBit, view, blockTransactions, mempool, (int)nOut));
    ...
    if (found)
    {
        if (nTimeBlock >= wallet.chain().getTip()->GetMedianTimePast()+1)
        {
            ...
            if (wallet.chain().getHeight().value_or(0) + 1 >= consensusParams.nOfflineStakeHeight)
            {
                ...
                // check block header
                return isSigned && CheckHeaderPoS(*pblock, consensusParams, wallet.chain().chainman());
            }
            else
            {
                ...
                return key.Sign(pblock->GetHashWithoutSign(), pblock->vchBlockSigDlgt) &&
                           ...
                           CheckHeaderPoS(*pblock, consensusParams, wallet.chain().chainman());
            }
        }
    }
...
CAmount GetTxGasFee(const CMutableTransaction& _tx, const CTxMemPool& mempool, CChainState& active_chainstate)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        QtumTxConverter convert(tx, mempool, NULL, NULL, contractflags);
