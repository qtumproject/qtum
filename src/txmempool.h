...
class CompareTxMemPoolEntryByEntryTimeOrGasPrice
{
public:
    bool operator()(const CTxMemPoolEntry& a, const CTxMemPoolEntry& b) const
    {
        ...
        // If neither of the txs we are comparing are contract txs, use the standard comparison based on entry time
...
struct entry_time_or_gas_price {};
...
public:
    ...
    using indexed_transaction_set = boost::multi_index_container<
        ...
        boost::multi_index::indexed_by<
            ...
            >,
            // sorted by fee rate with gas price (if contract tx) or entry time otherwise
            boost::multi_index::ordered_non_unique<
                boost::multi_index::tag<entry_time_or_gas_price>,
                boost::multi_index::identity<CTxMemPoolEntry>,
...
public:
    ...
    public:
        ...
        TxHandle StageAddition(const CTransactionRef& tx, CAmount fee, int64_t time, unsigned int entry_height, uint64_t entry_sequence, bool spends_coinbase, int64_t sigops_cost, LockPoints lp, CAmount min_gas_price = 0);
