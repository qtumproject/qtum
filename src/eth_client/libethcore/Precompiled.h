...
using PrecompiledExecutor = std::function<std::pair<bool, bytes>(bytesConstRef _in, 
    ChainOperationParams const& _chainParams, u256 const& _blockNumber)>;
...
#define ETH_REGISTER_PRECOMPILED(Name) static std::pair<bool, bytes> __eth_registerPrecompiledFunction ## Name(bytesConstRef _in, ChainOperationParams const& _chainParams, u256 const& _blockNumber); static PrecompiledExecutor __eth_registerPrecompiledFactory ## Name = ::dev::eth::PrecompiledRegistrar::registerExecutor(#Name, &__eth_registerPrecompiledFunction ## Name); static std::pair<bool, bytes> __eth_registerPrecompiledFunction ## Name
