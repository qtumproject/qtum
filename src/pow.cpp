...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params, bool fProofOfStake)
{
    ...
    if (nHeight < params.QIP9Height) {
        ...
        if((nHeight-(params.nReduceBlocktimeHeight + params.nRBTPowTargetBlockspan)) % params.nRBTPowTargetBlockspan == 0
                || nActualSpacing <= 2/3 * params.nRBTPowTargetBlockspan * nTargetSpacing || nActualSpacing >= 4/3 * params.nRBTPowTargetBlockspan * nTargetSpacing){
            if (nActualSpacing < 0)
            ...
            bnNew = (bnNew / (params.nRBTPowTargetTimespan+1)) * ((nActualSpacing * (params.nRBTPowTargetTimespan-1)) / (params.nRBTPowTargetBlockspan * nTargetSpacing));
        }
    }
