...
SendToContract::SendToContract(const PlatformStyle* platformStyle, QWidget* parent) : QWidget(parent),
...
void SendToContract::setModel(WalletModel* _model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonSendToContract->setToolTip(tr("Creates a Partially Signed Qtum Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
    }
}
